...
const Sidebar = () => {
    ...
    const leaveLabel = session?.user?.role === 'ADMIN' ? 'Leave Approvement' : 'Leave Application';
    ...
    ].filter(item => !(item.name === 'Activities' && session?.user?.role !== 'ADMIN'));

    return (
        <aside className="w-64 bg-[#2563eb] text-white flex flex-col h-screen transition-all duration-300 relative overflow-hidden shrink-0">
            {/* Logo Area */}
